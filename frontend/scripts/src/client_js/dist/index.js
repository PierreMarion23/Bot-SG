...
var init = function init() {
	...
	var success = function success(response) {
		...
		_fs2.default.writeFile('structure.json', JSON.stringify(data['types'], null, '\t'), function (err) {
				...
				var create_and_fill_dir = function create_and_fill_dir() {
					_fs2.default.mkdir('data/' + db_name, function (err) {
						...
						Object.keys(db_cat_columns).forEach(function (db_cat_name) {
							_fs2.default.writeFile('data/' + db_name + '/' + db_cat_name + '.txt', db_cat_columns[db_cat_name], function (err) {
